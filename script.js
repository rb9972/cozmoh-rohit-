...
templates.forEach(t => {
    ...
    const thumbnail = `https://image.thum.io/get/width/600/crop/800/${t.url}`;
...
document.getElementById("searchInput").addEventListener("input", function () {
    ...
    cards.forEach(card => {
        ...
        // ❗ Instead of display:block (which breaks grid)
        // we use display:flex (grid-safe) OR empty value to reset
        if (name.includes(term)) {
            card.style.display = "";   // ← keeps original grid layout perfect
        } else {
